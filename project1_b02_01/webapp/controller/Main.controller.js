sap.ui.define([
	...
	return Controller.extend("project1b0201.controller.Main", {

		onInit: function() {
		
		},
		onclick : function() {
		var oTxt = this.byId("input1");
		var sTxt = oTxt.getValue();

		var tTid = this.byId("string"); //대소문자 구문하여 실행 구문 하지 않으면 에러 발생 실행 안됨
		tTid.setText(sTxt);
         alert(sTxt);
	    
     },
	 onclick1 : function() {
		var oText = this.byId("input1"); // 컨트롤러의 Input객체 지정.
        var sText = oText.getValue(); // Input의 메소드 사용

        var tText = this.byId("string"); // Text 객체 지정 
		...
		initSampleDataModel: function() {
			...
			jQuery.ajax(sap.ui.require.toUrl("C:\Users\soldesk\Downloads\sap.ui.table.sample.Basic\mockdata\products.json"), {
				dataType: "json",
